...
module.exports = (function(){
	...
	self.index = function(opts, cb){

		/* ----------------- SET UP ----------------- */
		//Set Up logging 
		var debug_name = 'index', 
			messages = [],
			log = require('debug')('goneGamer:index:'+debug_name);
		logr(log, messages, '/lib/homeval.js -> '+debug_name+' accessed.');

		return cb({title: 'Test Page'});
	}

	/**
	 * [example description]
	 * @param  {[type]}   opts [description]
	 * @param  {Function} cb   The Callback Function to return to Express
	 */
	self.example = function (opts, cb) {
		cb({});		
	};
